        ...
        <>
            ...
            <ul className="list-group">
                {items.map((item, index) =>(
                    <li key={index} onClick={()=>{setSelectedIndex(index); setAlertVisible(true)}} className={selectedIndex === index ? 'list-group-item list-group-item-action list-group-item-success active' : 'list-group-item list-group-item-action list-group-item-success' }>{item[0]}</li>
                ))}
            </ul>
        </>
    )
